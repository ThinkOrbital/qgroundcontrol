...
Map {
    ...
    signal mapDoubleClicked(var position)
    ...
    MultiPointTouchArea {
        ...
        onReleased: (touchPoints) => {
            ...
            if (dragActive) {
                ...
                clickCount ++
                lastClickX = touchPoints[0].x
                lastClickY = touchPoints[0].y
                if(clickCount == 1) {
                    doubleClickTimer.start()
                } else if (clickCount >= 2) {
                    doubleClickTimer.stop()
                    clickCount = 0
                    mapDoubleClicked(Qt.point(lastClickX, lastClickY))
                }
        ...
        Timer {
            ...
            repeat: false
            onTriggered: {
                let count = multiTouchArea.clickCount
                multiTouchArea.clickCount = 0
                if(count === 1){
                    mapClicked(Qt.point(multiTouchArea.lastClickX, multiTouchArea.lastClickY))
                }
            }
        }
