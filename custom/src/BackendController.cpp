    ...
    if(message.sysid == SYSID_EMITTER_COMP || message.sysid == SYSID_DETECTOR_COMP)
    {
        ...
        switch (message.msgid) {
            ...
            case MAVLINK_MSG_ID_MSG_ACK: {
                ...
                qDebug() << "Received MSG_ACK from sysid " << message.sysid << " with type " << +msg_ack.ack_type;

                this->msg_ack_map_[message.sysid] = static_cast<AckType> (msg_ack.ack_type);

                if(this->subscribed_map_[message.sysid] && this->subscribed_map_[message.sysid])
                {
                ...
                else if(this->subscribed_map_[message.sysid] || this->subscribed_map_[message.sysid])
                {
                    if((this->msg_ack_map_[SYSID_EMITTER_COMP] == AckType::ack_target) || (this->msg_ack_map_[SYSID_DETECTOR_COMP] == AckType::ack_target))
                    {
                        this->targMsgSent_ = true;
        ...
        for(const auto& [sysid, state] : this->uav_state_map_)
        {
            ...
            switch(this->uav_state_map_[sysid])
            {
                case 0: //init state
                {
                    ...
                    {
                        if((this->uav_state_map_[sysid] == 0) && (this->uav_state_map_[sysid] == 0))
                        {
                            ...
                            // }
                            if(calMsgSent_ && (this->uav_state_map_[sysid] == 0) && (this->uav_state_map_[sysid] == 0))
                            {
                                this->setStartScanButtonEn(true);
                                this->setScanSettingsButtonEn(true);
                            }
                ...
                case 1: //takeoff state
                    ...
                    this->setScanSettingsButtonEn(false);
                ...
                case 5: //scan state
                    ...
                    switch(this->scan_state_)
                    {
                        ...
                        case StartScan::scan_tube_season:
                            ...
                            break;
                        default:
                            this->setFlightStatus("Unkown Scan State!");
                            break;
...
void BackendController::setScanSettingsButtonEn(const bool enabled)
{
    if(this->isScanSettingsButtonEn_ != enabled)
    {
        this->isScanSettingsButtonEn_ = enabled;
        emit scanSettingsButtonChanged();
    }
}
...
void BackendController::scanSettings()
{
    qDebug() << "Scan Settings Button Pressed";
    scan_state_ = StartScan::scan_settings;
    uint8_t scan_state = static_cast<uint8_t>(StartScan::scan_settings);
    this->sendStartScan(scan_state);
}
